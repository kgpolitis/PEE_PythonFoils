...
class Doc:
    ...
    def add(self,FS,name='Foil Section'):
        ...
        self.Docu.addObject("Part::Feature",name).Shape=Part.Wire(FS.Edges)
...
def WAGEN(Z,EAR,rR,tLE=0.,tTE=0,s=1e-4,nps=40,npp=40,Tvp=1,Tvs=1):
    ...
    Foil_Section=Part.Shape([spP,spS])
    
    return Foil_Section
    

def NACA(code,nps=40,npp=40,Tvp=1,Tvs=1):
    """ Foil Section Generating Function
        Inputs :
            -Specific for NACA 
...
def make_splines(afoil,nps=40,npp=40,Tvp=1,Tvs=1):
    ...
    vLEp = Base.Vector(vecLE[0],vecLE[1])
    vLEs = Base.Vector(vecLE[2],vecLE[3])
