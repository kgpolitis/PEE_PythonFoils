%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Plot Wageningen Blade Sections Wrapped in Cylinder
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
xfacew_c=0.5*r_R/c_D_r_R*sin(phi);
zfacew_c=-0.5*r_R/c_D_r_R*(1-cos(phi));
phi=2*xback_c_rot*c_D_r_R/r_R;
xbackw_c=0.5*r_R/c_D_r_R*sin(phi);
zbackw_c=-0.5*r_R/c_D_r_R*(1-cos(phi));

% make figures
figure
plot(xface_c_rot,yface_c_rot)
hold
plot(xback_c_rot,yback_c_rot)
plot3(xfacew_c,yface_c_rot,zfacew_c,'b','LineWidth',2)
plot3(xbackw_c,yback_c_rot,zbackw_c,'r','LineWidth',2)
axis equal      
legend('face','back','face-wrapped','back-wrapped')
grid on
xlabel('x/c')
ylabel('y/c')
zlabel('(z-r)/c')
