...
class BseriesFoil(foil):
    ...
    def __init__(self,Z,EAR,rR,tLE_tmax=0.2,tTE_tmax=0,Use_Original=1,Use_Smooth=0,s=0,Smooth_LE=0,x0s=0.1,x0p=0.1,ks=0.8,kp=0.8):
        ...
        # Smooth Leading Edge group
        self.tLE_tmax=tLE_tmax
        self.tTE_tmax=tTE_tmax
        self.Smooth_LE=Smooth_LE
        if (tTE_tmax>0 and (tLE_tmax>0 and not Smooth_LE)):
            self.closed(TE_closed=False,LE_closed=False)
        elif (tTE_tmax==0 and (tLE_tmax>0 and not Smooth_LE)):
            self.closed(LE_closed=False)
        elif (tTE_tmax>0 and (tLE_tmax==0 or Smooth_LE)):
            self.closed(TE_closed=False)
        elif (tTE_tmax==0 and (tLE_tmax==0 or Smooth_LE)):
            self.closed() # default arguments imply a closed foil
        
        # Spline Construction Group
