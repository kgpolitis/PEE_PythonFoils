...
#      
#    - Rework "closed" mechanics (see function : closed) 
#    
###
...
class afoil(ABC):
    ...
    def wrapXs(self,x,r,pitch,skew,rake):
        """ Cylindrical Wrapping of the foil given pitch, skew and rake at radius r"""
        return
    ...
    def PressureSide_FCAD(self,npoints=30,LE2TE=False):
        """ Generates lists of npoints (default=20) as X,Y coordinates to define FreeCAD base vectors for the suction side (oriented from LE to TE) """
        from numpy import linspace
        if (LE2TE):
            x=linspace(0,1,npoints)
